...
if sekcja == 'Analiza statystyczna':
    ...
    if (imie1 in list(im['Imię']))  and (imie2 in list(im['Imię']) ):
	    st.plotly_chart(px.line(imionka3.sort_values(['Rok','Imię']),x=sorted(list(imionka3['Rok'])),y='Liczba',color='Imię',width=1100, height=600, barmode='group').update_xaxes(tickmode='array').update_yaxes(rangemode='tozero'))
    else:
