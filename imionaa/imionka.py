...
if sekcja == 'Strona główna':
    ...
    #st.plotly_chart(px.line(im[im['Imię']==imie].sort_values(['Rok','Płeć']),x='Rok',y='Liczba',color='Płeć',markers=True,width=1100, height=600,color_discrete_sequence=[kol[im[im['Imię']==imie]['Płeć'].iloc[0]]]).update_yaxes(rangemode='tozero').update_traces(line_color='#0000ff', line_width=2))
    st.write([kol[im[im['Imię']==imie]['Płeć'].iloc[0]]])
...
if sekcja == 'Analiza statystyczna':
    ...
    if wybrany == "2020":
        ...
        col2.plotly_chart(px.bar(x=DF_2020_m[DF_2020_m['Województwo']==mies]['Liczba imion'][::-1],y=DF_2020_m[DF_2020_m['Województwo']==mies]['Imię'][::-1],
			     orientation='h',text=DF_2020_m[DF_2020_m['Województwo']==mies]['Liczba imion'][::-1],color=["red"]*10,
			     color_discrete_map="identity",
			     title='Top 10 imion męskich').update_xaxes(title_text='Liczba imion').update_yaxes(title_text='Imię'
				).update_layout(plot_bgcolor='white',title_x=0.5,height=600))
    if wybrany == "2019":
