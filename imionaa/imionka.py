...
if sekcja == 'Strona główna':
    ...
    st.dataframe(im[im['Imię']==imie].sort_values(['Rok','Płeć'])['Płeć'])
    st.dataframe([np.array(im[im['Imię']==imie].sort_values(['Rok','Płeć'])['Płeć'])[index] for index in sorted(np.unique(np.array(im[im['Imię']==imie].sort_values(['Rok','Płeć'])['Płeć']), return_index=True)[1])])
    st.plotly_chart(px.line(im[im['Imię']==imie].sort_values(['Rok','Płeć']),x='Rok',y='Liczba',color='Płeć',markers=True,width=1100, height=600,color_discrete_sequence=list(map(lambda x: kol[x],[np.array(im[im['Imię']==imie].sort_values(['Rok','Płeć'])['Płeć'])[index] for index in sorted(np.unique(np.array(im[im['Imię']==imie].sort_values(['Rok','Płeć'])['Płeć']), return_index=True)[1])]))).update_yaxes(rangemode='tozero').update_traces(line_width=2))
    st.dataframe(im[im['Imię']==imie].sort_values(['Rok','Płeć'])['Płeć'])
...
if sekcja == 'Analiza korespondencji':
    ...
    if roczek_wybrany == "2019":
        ...
        st.image('imionaa/AK/f2_chl2019.png', width=1000)
